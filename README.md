...
- Человекочитаемые консольные логи с псевдографикой, цветом при поддержке терминала и Windows-safe fallback.
...
`setup.bat` делает то же для Windows и дописывает недостающие новые настройки в существующий `.env`.
...
На Windows используйте `run.bat`; пункты меню совпадают с `run.sh`.
`run.bat` включает UTF-8 для Python. Цветные ANSI-логи включаются только если Windows-консоль их поддерживает; иначе используется безопасная ASCII-псевдографика.
