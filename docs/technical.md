...
| `LLM_MAX_TICKERS` | `8` | Сколько торговых решений модель может вернуть за один запрос |
...
3. Модель возвращает строгий JSON с решениями `BUY`, `SELL` или `HOLD`.
...
Уровень `1..5` влияет на размер заявки, интервал следующего LLM/биржевого цикла и число исполняемых LLM-решений. Текущий уровень периодически выводится в логах рядом со статусом риск-менеджера.
